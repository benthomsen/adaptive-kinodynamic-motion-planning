...
%		u(4)    = 		Throttle, dT, %
%		u(5)    =		Stabilator, dSr, rad

load('Fmodel.mat');
c123 = Fmodel(:,1:3); c456 = Fmodel(:,4:6); Fmodel2 = [c456, c123, Fmodel(:,7:end)];
r123 = Fmodel2(1:3,:); r456 = Fmodel2(4:6,:); A = [r456; r123; Fmodel2(7:end,:)];

load('Gmodel.mat');
Gr123 = Gmodel(1:3,[1:4, 7]);
Gr456 = Gmodel(4:6,[1:4, 7]);
B = [Gr456; Gr123; Gmodel(7:end,[1:4, 7])];

u0 = [0; 0; 0; 0.1855; -0.0399];
c = B * u0; % trim condition

B = B(:,1:4);

start = [0, 0, -3048, 95.766, 0, 7.1259, 0, 0, 0, 0, 0.0743, 0];
goal  = [3000, 0, -3048, 95.766, 0, 7.1259, 0, 0, 0, 0, 0.0743, 0];
...
rrt = rrtstar_v4(A, B, c, R, start', goal', 150);
