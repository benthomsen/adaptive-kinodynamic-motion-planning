...
%         min_dist = 9e99;
    ...
    methods
        function obj = rrtstar_v4_mod(A, B, c, R, start, goal, max_rad, step_size) % this will only work for linear systems
...
%             end
        ...
        function [cost, times, states, u] = calc_numerical(obj, x0, x1, A, B)
%             if ~exist('A', 'var')
%                 A = obj.A_nodes(:,:,1);
%                 B = obj.B_nodes(:,:,1);
%             end
            ...
            [t, X, ~, ~, ~] = ode45(@sys_dynamics, 0:obj.step_size:500, [zeros(dim^2,1); x0], opts, A, B, obj.c_, obj.R_, x1);
            tau_star = t(end);
            if tau_star < obj.step_size*2
            	[t, X, ~, ~, ~] = ode45(@sys_dynamics, 0:obj.step_size*0.01:500, [zeros(dim^2,1); x0], opts, A, B, obj.c_, obj.R_, x1);
                tau_star = t(end);
                X_fin = X(end,:);
                ...
                [times_backwards, X2] = ode45(@state_solution, tau_star:-obj.step_size*0.01:0, [x1; d_star], opts2, A, B, obj.c_, obj.R_);
                times = flip(times_backwards);
                states = flip(X2(:,1:dim));
                u = zeros(length(times),size(B,2));
                ...
                opts2 = odeset('AbsTol',1e-3,'RelTol',1e-3);
                [times_backwards, X2] = ode45(@state_solution, tau_star:-obj.step_size:0, [x1; d_star], opts2, A, B, obj.c_, obj.R_);
                times = flip(times_backwards);
                states = flip(X2(:,1:dim));
                u = zeros(length(times),size(B,2));
                for i = 1:length(times)
...
%             warnStruct = warning('off',msgId);
        ...
        % rrtstar.run:
        function [path_states, closest_end_state] = run(obj, sample_free_state, is_state_free, is_input_free, start, goal, display, linstate)
            T = [start]; % tree
            costs = [0];
            times = [0];
            parents = [-1];
...
%             [A, B] = linstate(obj, start);
%             obj.A_nodes(:,:,1) = obj.A_;
%             obj.B_nodes(:,:,1) = obj.B_;
            A = obj.A_nodes(:,:,1);
            B = obj.B_nodes(:,:,1);
            [cost, ~, states, u] = calc_numerical(obj, start, goal, A, B);
%             time = times_i(end);
...
%                     x_i = sample_free_state(obj.x0); % sample a random state within bounds and outside of obstacles
                    ...
                    while ~isempty(queue)
                        ...
                        A = obj.A_nodes(:,:,node_idx);
                        B = obj.B_nodes(:,:,node_idx);
                        [cost, times_i, states, u] = calc_numerical(obj, node, x_i, A, B);
                        time = times_i(end);
...
%                             end
                            ...
                            if  free_state && free_input
                                ...
                                [obj.A_nodes(:,:,end+1), obj.B_nodes(:,:,end+1)] = linstate(x_i);
                                continue;
                            end
                        end
                ...
                while ~isempty(stack)
                    ...
                    [cost, ~, states, u] = calc_numerical(obj, x_i, state, obj.A_nodes(:,:,node.index), obj.B_nodes(:,:,node.index));
%                     time = times_i(end);
                ...
                [cost, times_i, states, u] = calc_numerical(obj, x_i, goal, obj.A_nodes(:,:,end), obj.B_nodes(:,:,end));
                time = times_i(end);
