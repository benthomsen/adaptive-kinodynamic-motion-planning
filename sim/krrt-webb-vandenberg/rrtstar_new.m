...
%         eval_time_for_distance_internal;
    ...
    methods
        function obj = rrtstar_new(A, B, c, R, start, goal) % this will only work for linear systems
        ...
        function [cost, times, states, u] = calc_numerical(obj, x0, x1)
            ...
            [t, X, ~, ~, ~] = ode45(@sys_dynamics, 0:0.1:60, [zeros(dim^2,1); x0], opts, obj.A_, obj.B_, obj.c_, obj.R_, x1);
            tau_star = t(end);
            X_fin = X(end,:);
