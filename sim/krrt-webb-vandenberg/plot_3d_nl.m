function [ scratch ] = plot_3d_nl(scratch, obj, tree, parents, obstacles, goal_state, goal_cost, goal_parent )
...
if goal_cost < inf
    p = goal_parent;
    h = draw_trajectory(obj, tree(:,p), goal_state, 'green', 3, p);
    uistack(h, 'top')
    ...
    while p > 0
        ...
        h = draw_trajectory(obj, tree(:,p), tree(:,c), 'green', 3, p);
        uistack(h, 'top')
        uistack(ptplot, 'top')
        c = p;
        p = parents(c);
...
function [h] = draw_trajectory(obj,x0,x1,color, thickness, p)
